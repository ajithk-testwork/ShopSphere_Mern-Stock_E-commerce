...
export const createCheckoutSession = async (req, res) => {
  try {
    ...
    const session = await stripe.checkout.sessions.create({
      ...
      cancel_url: `https://shop-sphere-mern-stock-e-commerce-v.vercel.app/payment-cancel?orderId=${orderId}`,
    });
...
export const verifyPayment = async (req, res) => {
  try {
    ...
    console.log("Payment status updated to paid:", order._id);
    ...
    sendEmail(
      ...
      paymentSuccessEmail(order.userInfo.name, order),
    ).catch((error) => {
...
export const paymentFailed = async (req, res) => {
  try {
    ...
    sendEmail(
      ...
      "ShopSphere Payment Unsuccessful ❌",
    ...
    res.json({
      success: true,
      message: "Payment failure email processed",
    });

  } catch (error) {
    console.error(
      "Payment failed controller error:",
      error
    );
