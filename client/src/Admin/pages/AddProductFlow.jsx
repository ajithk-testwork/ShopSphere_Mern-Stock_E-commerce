...
export default function AddProductFlow() {
  ...
  useEffect(() => {
    const fetchCategories = async () => {
      try {
        const res = await api.get("/categories"); // Simplified call
    ...
    if (isNewCategory) {
      ...
      try {
        const res = await api.post("/categories/create", formData, {
          headers: { "Content-Type": "multipart/form-data" }, // No manual Auth header needed
        });
      ...
      <header className="mb-10">
        ...
        <h2 className="text-4xl font-black text-gray-900 leading-tight">
          {step === 1
            ...
            : `Step 2: Add Products to ${createdCategory?.name}`}
        </h2>
      </header>
            ...
            <form onSubmit={handleCategoryStep}>
              {!isNewCategory ? (
                ...
                <div className="space-y-6">
                  <div>
                    <label className="text-[10px] font-bold uppercase text-gray-400 mb-2 block ml-1">
                      New Category Name
                    </label>
                    <input
                      ...
                      {catPreview ? (
                        <div className="relative w-full h-full group">
                          ...
                          <button
                            ...
                            className="absolute top-2 right-2 p-1.5 bg-white rounded-full shadow-lg text-red-500 hover:scale-110 transition-transform cursor-pointer"
                          >
                            <X size={16} />
                          </button>
                        </div>
              ...
              <button
                type="submit"
                disabled={loading}
                className="w-full mt-10 bg-blue-600 text-white py-5 rounded-2xl font-bold flex items-center justify-center gap-2 hover:bg-blue-700 hover:shadow-lg hover:shadow-blue-200 transition-all cursor-pointer"
              >
                {loading ? (
            ...
            <div className="bg-white p-6 rounded-[2rem] shadow-sm border border-gray-100">
              ...
              <div className="space-y-4">
                  ...
                  {productForm.preview ? (
                    <div className="relative w-full h-full">
                      ...
                      <button
                        ...
                        className="absolute top-1 right-1 p-1 bg-white rounded-full text-red-500 cursor-pointer"
                      >
                        <X size={12} />
                      </button>
                    </div>
                ...
                <button
                  ...
                  className="w-full bg-blue-50 text-blue-600 py-3 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-blue-100 transition-all cursor-pointer active:scale-95"
                >
          ...
          <div className="lg:col-span-2 space-y-6">
                  ...
                  <tbody className="divide-y divide-gray-100">
                    ...
                    {productList.length === 0 && (
                      <tr>
                        <td
                          colSpan="3"
                          className="px-6 py-20 text-center text-gray-300 font-medium italic text-sm"
                        >
                          Table is empty. Add a product on the left to begin.
                        </td>
                      </tr>
                    )}
                  </tbody>
            ...
            <div className="flex flex-col gap-3">
              <button
                ...
                className="w-full bg-black text-white py-5 rounded-2xl font-bold text-lg flex items-center justify-center gap-3 shadow-xl hover:bg-gray-900 active:scale-[0.99] disabled:bg-gray-200 disabled:shadow-none transition-all cursor-pointer"
              >
                {loading ? (
                ...
                {loading
                  ? "Publishing Database..."
                  : `Finalize & Publish to ${createdCategory?.name}`}
              </button>
              <button
                onClick={() => setStep(1)}
                className="text-[10px] font-black uppercase text-gray-400 tracking-widest hover:text-gray-600 transition-colors cursor-pointer"
              >
