        ...
        >
          ...
          <div className="bg-white p-8 sm:p-10 rounded-3xl shadow-sm border border-slate-200">
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-x-8 gap-y-6">
              ...
              <div className="space-y-6">
                <div>
                  <label className="text-xs font-bold uppercase text-slate-500 mb-2 block ml-1">Product Name</label>
                  <input
                    ...
                    className="w-full px-4 py-3.5 rounded-xl bg-slate-50 border border-slate-200 font-medium text-sm outline-none focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 placeholder-slate-400 transition-all"
                  />
                </div>
                
                <div>
                  <label className="text-xs font-bold uppercase text-slate-500 mb-2 block ml-1">Description</label>
                  <textarea
                    ...
                    className="w-full px-4 py-3.5 rounded-xl bg-slate-50 border border-slate-200 font-medium text-sm h-32 resize-none outline-none focus:bg-white focus:border-indigo-500 focus:ring-4 focus:ring-indigo-500/10 placeholder-slate-400 transition-all"
                  />
                </div>
              </div>
              ...
              <div className="space-y-6 flex flex-col justify-between">
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-xs font-bold uppercase text-slate-500 mb-2 block ml-1">Price (₹)</label>
                    <input
                      placeholder="0.00"
                      type="number"
                  ...
                  <div>
                    <label className="text-xs font-bold uppercase text-slate-500 mb-2 block ml-1">Stock</label>
                    <input
                      placeholder="0"
                      type="number"
                ...
                <div>
                   <label className="text-xs font-bold uppercase text-slate-500 mb-2 block ml-1">Product Image</label>
                  <div className="relative h-[128px] rounded-xl bg-slate-50 border-2 border-dashed border-slate-200 flex items-center justify-center overflow-hidden hover:bg-slate-100 transition-colors group">
                    {productForm.preview ? (
                      <div className="relative w-full h-full">
                        <img
            ...
            <div className="mt-8 pt-8 border-t border-slate-100 flex justify-end">
              <button
                ...
                className="px-6 py-3 bg-indigo-50 text-indigo-600 rounded-xl font-bold flex items-center justify-center gap-2 hover:bg-indigo-100 transition-all cursor-pointer active:scale-[0.98]"
              >
          ...
          <div className="bg-white rounded-3xl shadow-sm border border-slate-200 overflow-hidden">
            <div className="px-8 py-6 border-b border-slate-100 flex justify-between items-center bg-white">
              <div>
                <h3 className="text-lg font-bold text-slate-900">Publishing Queue</h3>
                <p className="text-sm text-slate-500 mt-1">Review items before saving to database</p>
              </div>
              <div className="bg-indigo-50 text-indigo-700 px-4 py-1.5 rounded-lg text-sm font-bold border border-indigo-100 flex items-center gap-2">
                <Package size={16}/>
            ...
            <div className="overflow-x-auto">
              <table className="w-full text-left border-collapse">
                <thead className="bg-slate-50/50 text-xs font-bold text-slate-500 uppercase tracking-wider border-b border-slate-100">
                  <tr>
                    <th className="px-8 py-4 font-semibold">Product Detail</th>
                    <th className="px-8 py-4 font-semibold">Price & Stock</th>
                      ...
                      >
                        ...
                        <td className="px-8 py-5 text-right">
                          <button
                            ...
                            className="p-2.5 text-slate-400 hover:text-red-600 hover:bg-red-50 rounded-xl transition-all cursor-pointer opacity-0 group-hover:opacity-100 focus:opacity-100"
                            title="Remove item"
                          >
                            <Trash2 size={20} />
                          </button>
                    ...
                    <tr>
                      ...
                      >
                        <div className="flex flex-col items-center justify-center text-slate-400">
                          ...
                          <p className="font-medium text-slate-500">Your queue is empty</p>
                          <p className="text-sm mt-1.5">Fill out the form above to add products here.</p>
                        </div>
                      </td>
                    </tr>
                  )}
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center justify-between bg-white p-5 rounded-3xl border border-slate-200 shadow-sm">
            <button
              onClick={() => setStep(1)}
              className="text-sm font-bold text-slate-500 hover:text-slate-800 transition-colors cursor-pointer flex items-center gap-2 px-4 py-2 rounded-xl hover:bg-slate-50"
            >
              <ChevronRight size={18} className="rotate-180" /> Back to Category
            </button>
            
            <button
              onClick={handleFinalSubmit}
              disabled={loading || productList.length === 0}
              className="w-full sm:w-auto px-10 py-4 bg-slate-900 text-white rounded-xl font-bold flex items-center justify-center gap-3 hover:bg-slate-800 active:scale-[0.98] disabled:bg-slate-200 disabled:text-slate-400 disabled:cursor-not-allowed transition-all shadow-sm"
            >
              {loading ? (
                <Loader2 className="animate-spin" size={20} />
              ) : (
